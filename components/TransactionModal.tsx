...
export const TransactionModal: React.FC<TransactionModalProps> = ({
  ...
  const TransactionRow: React.FC<{ tx: YNABTransaction }> = ({ tx }) => {
    ...
    return (
      <div
        {...longPressHandlers}
        className="flex items-center justify-between p-3 bg-surface-2 rounded-xl hover-surface-2 transition-colors min-h-[60px] cursor-pointer select-none long-press-target"
      >
        <div className="flex-1 min-w-0 mr-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[15px] font-medium text-strong truncate">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {tx.memo && (
              <>
                ...
                <span className="text-[12px] text-muted truncate max-w-[120px]">
                  {tx.memo}
                </span>
              </>
            )}
        ...
        <span className="text-[15px] font-semibold text-strong whitespace-nowrap">
  ...
  return (
    <>
      ...
      >
        <div
          ref={modalRef}
          className="relative w-full sm:max-w-lg max-h-[85vh] bg-white dark:bg-surface-1 rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden animate-slide-up"
          role="dialog"
          aria-modal="true"
          aria-labelledby="modal-title"
        >
          {/* Header */}
          <div className="sticky top-0 z-10 bg-white/80 dark:bg-surface-1/80 backdrop-blur-xl border-b border-black/5 dark:border-white/10">
            <div className="flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 id="modal-title" className="text-[17px] font-bold text-strong">
                    {categoryName}
                  </h2>
              ...
              <button
                onClick={onClose}
                className="w-11 h-11 flex items-center justify-center hover-surface-2 rounded-full transition-colors -mr-2 -mt-1"
                aria-label="Kapat"
              >
