...
export const MerchantParetoChart: React.FC<MerchantParetoChartProps> = ({ data, total }) => {
  ...
  return (
    <div className="chart-panel p-4">
      ...
      <div className="flex flex-wrap gap-2 text-[11px] font-semibold text-muted uppercase tracking-[0.14em]">
        <span className="chip">Harcama</span>
        <span className="chip">Kümülatif</span>
      </div>
      <div className="h-56">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'var(--text-muted)' }} />
            <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickFormatter={(v) => `₺${Math.round(v / 1000)}k`} />
            <YAxis yAxisId="right" orientation="right" domain={[0, 100]} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickFormatter={(v) => `${v}%`} />
            <Tooltip
            ...
            />
            <Bar yAxisId="left" dataKey="amount" fill="var(--brand-1)" radius={[8, 8, 0, 0]} />
            <Line yAxisId="right" type="monotone" dataKey="cumulative" stroke="var(--accent-warm)" strokeWidth={2} dot={{ r: 3 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
